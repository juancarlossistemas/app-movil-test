...
--==============================================================================
CREATE PROCEDURE PROC_TG_TRATAMIENTO_LISTAR (
@USUARI_N_CODIGO INT = 0
)
AS
BEGIN
	DECLARE @NIVANS_N_CODIGO INT = (SELECT NIVANS_N_CODIGO FROM TG_DIAGNOSTICO WHERE USUARI_N_CODIGO = @USUARI_N_CODIGO AND TEST_N_CODIGO = 1)

	SELECT 
	TRATAM.TRATAM_N_CODIGO,
	NIVANS.NIVANS_T_DESCRI,
	SESION.SESION_T_DESCRI,
	SEMANA.SEMANA_T_DESCRI,
	SEMANA.SEMANA_N_CANDIA,
	TECNIC.TECNIC_T_TITULO
	FROM		TG_TRATAMIENTO		TRATAM
	INNER JOIN	TG_NIVEL_ANSIEDAD	NIVANS	ON	NIVANS.NIVANS_N_CODIGO	=	TRATAM.NIVANS_N_CODIGO	AND	NIVANS.NIVANS_F_ACTIVO = 1
	INNER JOIN	TG_SESION			SESION	ON	SESION.SESION_N_CODIGO	=	TRATAM.SESION_N_CODIGO	AND	SESION.SESION_F_ACTIVO = 1
	INNER JOIN	TG_SEMANA			SEMANA	ON	SEMANA.SEMANA_N_CODIGO	=	TRATAM.SEMANA_N_CODIGO	AND	SEMANA.SEMANA_F_ACTIVO = 1
	INNER JOIN	TG_TECNICA			TECNIC	ON	TECNIC.TECNIC_N_CODIGO	=	TRATAM.TECNIC_N_CODIGO	AND	TECNIC.TECNIC_F_ACTIVO = 1
	WHERE
	NIVANS.NIVANS_N_CODIGO = @NIVANS_N_CODIGO AND TRATAM.TRATAM_F_ACTIVO = 1
END
GO

CREATE PROCEDURE PROC_TG_TECNICA_LISTAR (
@USUARI_N_CODIGO INT,
@TRATAM_N_CODIGO INT
)
AS
BEGIN
	SELECT
	TECNIC.TECNIC_N_CODIGO,
	TECNIC.TECNIC_T_TITULO,
	TECDET.TIPCON_N_CODIGO,
	TECDET.TECDET_T_CONTEN,
	*
	FROM		TG_TECNICA			TECNIC
	INNER JOIN	TG_TECNICA_DETALLE	TECDET	ON	TECDET.TECNIC_N_CODIGO	=	TECNIC.TECNIC_N_CODIGO	AND	TECDET.TECDET_F_ACTIVO = 1
	WHERE TECNIC.TECNIC_N_CODIGO = (SELECT TECNIC_N_CODIGO FROM TG_TRATAMIENTO WHERE TRATAM_N_CODIGO = @TRATAM_N_CODIGO)

	INSERT INTO TG_TRATAMIENTO_HISTORIAL (USUARI_N_CODIGO, TRATAM_N_CODIGO) VALUES (@USUARI_N_CODIGO, @TRATAM_N_CODIGO)
END
GO

--====================================================================================

CREATE PROCEDURE PROC_TG_DIAGNOSTICO_CONSULTAR (
@USUARI_N_CODIGO INT,
@TEST_N_CODIGO INT = 0
)
AS
BEGIN
	...
	INNER JOIN TG_PUNTUACION_CONDUCTUAL		PUNCON	ON	PUNCON.PUNCON_N_CODIGO	=	DIAGNO.PUNCON_N_CODIGO	AND	PUNCON.PUNCON_F_ACTIVO	=	1
	INNER JOIN TG_PUNTUACION_DIRECTA		PUNDIR	ON	PUNDIR.PUNDIR_N_CODIGO	=	DIAGNO.PUNDIR_N_CODIGO	AND	PUNDIR.PUNDIR_F_ACTIVO	=	1
	INNER JOIN TG_TEST						TEST	ON	TEST.TEST_N_CODIGO		=	DIAGNO.TEST_N_CODIGO	AND	TEST_F_ACTIVO			=	1
	WHERE
	DIAGNO.USUARI_N_CODIGO = @USUARI_N_CODIGO AND DIAGNO.DIAGNO_F_ACTIVO = 1
	AND (DIAGNO.TEST_N_CODIGO = @TEST_N_CODIGO OR @TEST_N_CODIGO = 0)
	--ORDER BY TEST_N_CODIGO DESC
END
GO